...
export default function ServiceCard({ num, label, desc, heroImage, slug }: Service) {
  return (
    <Link
      href={`/services/${slug}`}
      className="group glass-card h-full rounded-card p-[1.3rem] flex flex-col transition-transform duration-300 hover:-translate-y-[6px] hover:border-white/[.28]"
    >
      ...
      <span className="mt-4 text-[.9rem] font-semibold text-white/80 group-hover:text-white inline-flex items-center gap-[.35rem]">
        En savoir plus →
      </span>
    </Link>
  );
